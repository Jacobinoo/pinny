...
export default function SaveToBoardButton({ pin, compact = false, suggestedBoardId }: SaveToBoardButtonProps) {
      ...
      <div style={{
        ...
        {boards.length === 0 ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', maxHeight: '350px', overflowY: 'auto', paddingRight: '8px' }}>
            {[...boards].sort((a, b) => {
              if (a.id === suggestedBoardId) return -1;
              if (b.id === suggestedBoardId) return 1;
              return 0;
            }).map(board => {
  ...
  return (
    <>
      <button 
        className={compact ? "save-btn-compact" : "save-btn"} 
        style={compact ? {
          ...
          backgroundColor: 'var(--accent)', color: '#fff', border: 'none', 
          padding: '12px 24px', borderRadius: '24px', fontSize: '1rem', 
